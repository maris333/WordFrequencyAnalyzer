...
class TestWebPageAnalyzer(unittest.TestCase):
    ...
    def test_extract_text(self):
        html_content = "<html><head><style>body{background-color:#f00;}</style><script>alert('Hello, World!');" \
                       "</script></head><body><h1>This is a short document</h1></body></html>"

        result = extract_text(html_content)

        self.assertEqual(result, 'This is a short document')
    ...
    def test_display_results(self, mock_print):
        ...
        expected_calls = [call('apple : 2'), call('banana : 2'), call('orange : 1')]
